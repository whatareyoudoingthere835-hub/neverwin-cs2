...
m_bWaitForNoAttack=0x1C98
m_bGunGameImmunity=0x3268
...
m_iClip1=0x1700
m_nNextPrimaryAttackTick=0x16F0
m_bInReload=0x1814
```
...
| F6 | Nonagon Ragebot + resolver + настраиваемый автоогонь |
| END | выгрузка DLL |

INSERT оставлен как запасной тоггл меню.

## Ragebot: живые цели и автоогонь

Ragebot больше не сканирует весь entity list как набор pawn. Игроки берутся из
64 слотов `CCSPlayerController`, pawn разрешается через текущий `m_hPlayerPawn`,
а цель считается живой только при одновременном совпадении
`m_bPawnIsAlive`, `m_iHealth > 0` и `m_lifeState == 0`. Перед выстрелом связь
controller → handle → тот же pawn проверяется ещё раз; dormant-цели и игроки со
spawn immunity отбрасываются. Эта строгая controller-проверка применяется только
к удалённым целям. Локальный `dwLocalPlayerPawn` проверяется напрямую по
health/lifeState, чтобы устаревшие controller-оффсеты не отключали F1/F2/recoil.

Опция **«Автоогонь / триггер»** находится внутри настроек Ragebot. Выстрел
происходит только после подтверждения pawn под прицелом (`m_iIDEntIndex`) или
когда остаточная угловая ошибка уже не больше 1.5°. Она также учитывает патроны,
перезарядку, `m_bWaitForNoAttack` и weapon cooldown через
`m_nNextPrimaryAttackTick <= m_nTickBase`. Mouse-down и mouse-up разнесены по
разным проходам цикла, поэтому клик не теряется между игровыми тиками. При
открытии меню, потере фокуса, отключении рейджа или выгрузке кнопка гарантированно
отпускается.
...
Обе версии наводятся на ближайшего **живого** тиммейта **через стены, на любой
дистанции**: видимость не проверяется, дальность не ограничена. Трупы и старые
pawn после респавна полностью исключены общей controller-based проверкой.
Цель — origin + 64 юнита (корпус/голова).
