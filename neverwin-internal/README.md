...
## Канал углов F1/F2 — прямые записи viewAngles + диагностика (v8+)

F1/F2 пишут `dwViewAngles` напрямую из цикла фич (1 мс) — тот же метод, что
работал в ранних версиях (F2 крутил камеру, значит запись доходит до игры).

Почему F1 молчал, а F2 работал: у F2 одно условие — живой враг (health/team,
стабильные чтения). У F1 три дополнительных, и все молчаливые: позиция глаз
(scene node → origin + viewOffset), живой тиммейт, сцена-нода тиммейта. Если
хоть одно падало — запись не выполнялась, и это никак не было видно.

Теперь каждый отказ пишет в `%TEMP%\neverwin.log` раз в 5 секунд:

- `F1: тиммейтов N, цель (x y z), углы (p, y)` — работает;
- `F1: живых тиммейтов не найдено` — наводиться не на кого;
- `F1: глаза не прочитались` — позиционные оффсеты стухли (обнови ini).

Канал через CreateMove (v6/v7) удалён: на клиенте из лога хук не встал
(`CreateMove (vtable 5) не захучен`), а его фолбэк жил внутри несостоявшегося
хука — F1/F2 были мертвы целиком.
