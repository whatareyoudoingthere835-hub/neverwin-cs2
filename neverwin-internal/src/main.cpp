...
namespace {
    ...
    DWORD WINAPI HackThreadStub(LPVOID param) {
        ...
        // Меню внешнее (D3D12-оверлей); Init всегда true, ветка — на случай
        // будущего отказа shared-памяти/оверлея.
        if (!gui::Init()) {
            NW_LOG(L"WARNING: меню недоступно — используй фичи с клавиатуры (F1-F5).");
        }
