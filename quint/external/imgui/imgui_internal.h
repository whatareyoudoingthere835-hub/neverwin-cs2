...
struct IMGUI_API ImDrawListSharedData
{
    ...
    float           LegacySize;                   // Current font size (used for for simplified AddText overload)
    float           FontScale;                  // Current font scale (== LegacySize / Font->LegacySize)
...
{
    ImFont*     Font;
    float       LegacySizeBeforeScaling;      // ~~ style.LegacySizeBase
    float       LegacySizeAfterScaling;       // ~~ g.LegacySize
};
...
struct ImGuiContext
{
    ...
    ImFontBaked*            FontBaked;                          // Currently bound font at currently bound size. (== Font->GetFontBaked(LegacySize))
    float                   LegacySize;                           // Currently bound font size == line height (== LegacySizeBase + externals scales applied in the UpdateCurrentLegacySize() function).
    float                   LegacySizeBase;                       // Font size before scaling == style.LegacySizeBase == value passed to PushFont() when specified.
    float                   FontBakedScale;                     // == FontBaked->Size / LegacySize. Scale factor over baked size. Rarely used nowadays, very often == 1.0f.
...
struct IMGUI_API ImGuiWindow
{
    ...
    float                   FontRefSize;                        // This is a copy of window->CalcLegacySize() at the time of Begin(), trying to phase out CalcLegacySize() especially as it may be called on non-current window.
...
public:
    ...
    // We don't use g.LegacySize because the window may be != g.CurrentWindow.
    ImRect      Rect() const            { return ImRect(Pos.x, Pos.y, Pos.x + Size.x, Pos.y + Size.y); }
    ImRect      TitleBarRect() const    { return ImRect(Pos, ImVec2(Pos.x + SizeFull.x, Pos.y + TitleBarHeight)); }
    ImRect      MenuBarRect() const     { float y1 = Pos.y + TitleBarHeight; return ImRect(Pos.x, y1, Pos.x + SizeFull.x, y1 + MenuBarHeight); }

    // [OBSOLETE] ImGuiWindow::CalcLegacySize() was removed in 1.92.0 because error-prone/misleading. You can use window->FontRefSize for a copy of g.LegacySize at the time of the last Begin() call for this window.
    //float     CalcLegacySize() const    { ImGuiContext& g = *Ctx; return g.LegacySizeBase * FontWindowScale * FontWindowScaleParents;
};
...
namespace ImGui
{
    ...
    IMGUI_API void          UpdateCurrentLegacySize(float restore_font_size_after_scaling);
    ...
    inline float            GetRoundedLegacySize(float size) { return IM_ROUND(size); }
