...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    const float new_ref_scale_unit = g.LegacySize; // g.Font->GetCharAdvance('A') ?
...
void ImGui::TableHeader(const char* label)
{
    ...
    {
        w_arrow = ImTrunc(g.LegacySize * ARROW_SCALE + g.Style.FramePadding.x);
        if (column->SortOrder != -1)
            sort_arrow = true;
...
void ImGui::TableAngledHeadersRowEx(ImGuiID row_id, float angle, float max_label_width, const ImGuiTableHeaderData* data, int data_count)
{
    ...
    const float header_height = g.LegacySize + g.Style.CellPadding.x * 2.0f;
    ...
    const float line_off_for_ascent_x = (ImMax((g.LegacySize - ascent_scaled) * 0.5f, 0.0f) / -sin_a) * (flip_label ? -1.0f : 1.0f);
    ...
    for (int pass = 0; pass < 2; pass++)
        for (int order_n = 0; order_n < data_count; order_n++)
        {
            ...
            if (pass == 0)
            {
                ...
                const float line_off_step_x = (g.LegacySize / -sin_a);
