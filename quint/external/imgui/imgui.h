...
namespace ImGui
{
    ...
    //  - PushFont(font, style.LegacySizeBase * 2.0f)  // Change font and set size to be twice bigger than current size.
    ...
    //  - CORRECT:   PushFont(NULL, style.LegacySizeBase)         // use current unscaled size    == does nothing
    //  - CORRECT:   PushFont(NULL, style.LegacySizeBase * 2.0f)  // use current unscaled size x2 == make text twice bigger
    //  - INCORRECT: PushFont(NULL, GetLegacySize())              // INCORRECT! using size after global factors already applied == GLOBAL SCALING FACTORS WILL APPLY TWICE!
    //  - INCORRECT: PushFont(NULL, GetLegacySize() * 2.0f)       // INCORRECT! using size after global factors already applied == GLOBAL SCALING FACTORS WILL APPLY TWICE!
    ...
    IMGUI_API float         GetLegacySize();                                                  // get current scaled font size (= height in pixels). AFTER global scale factors applied. *IMPORTANT* DO NOT PASS THIS VALUE TO PushFont()! Use ImGui::GetStyle().LegacySizeBase to get value before global scale factors.
    IMGUI_API ImFontBaked*  GetFontBaked();                                                 // get current font bound at current size // == GetFont()->GetFontBaked(GetLegacySize())
    ...
    IMGUI_API float         GetTextLineHeight();                                            // ~ LegacySize
    IMGUI_API float         GetTextLineHeightWithSpacing();                                 // ~ LegacySize + style.ItemSpacing.y (distance in pixels between 2 consecutive lines of text)
    IMGUI_API float         GetFrameHeight();                                               // ~ LegacySize + style.FramePadding.y * 2
    IMGUI_API float         GetFrameHeightWithSpacing();                                    // ~ LegacySize + style.FramePadding.y * 2 + style.ItemSpacing.y (distance in pixels between 2 consecutive lines of framed widgets)
    ...
    IMGUI_API float         GetTreeNodeToLabelSpacing();                                        // horizontal distance preceding label when using TreeNode*() or Bullet() == (g.LegacySize + style.FramePadding.x*2) for a regular unframed TreeNode
...
{
    // Font scaling
    // - recap: ImGui::GetLegacySize() == LegacySizeBase * (FontScaleMain * FontScaleDpi * other_scaling_factors)
    float       LegacySizeBase;               // Current base font size before external global factors are applied. Use PushFont(NULL, size) to modify. Use ImGui::GetLegacySize() to obtain scaled value.
    ...
    float       IndentSpacing;              // Horizontal indentation when e.g. entering a tree node. Generally == (LegacySize + FramePadding.x*2).
    ...
    float       _NextFrameLegacySizeBase;     // FIXME: Temporary hack until we finish remaining work.
...
struct ImColor
{
    ...
    inline ImColor AlphaOverride(float alpha) {
        ImColor out = Value;
        out.Value.w *= alpha;
        return out;
    }
...
struct ImFontConfig
{
    ...
    //unsigned int  FontLoaderFlags;       // --       // [Renamed in 1.92] Ue FontLoaderFlags.
...
#endif
    //unsigned int                      FontLoaderFlags;        // OBSOLETED in 1.92.0: Renamed to FontLoaderFlags.
...
struct ImFontBaked
{
    ...
    float                       Ascent, Descent;    // 4+4   // out // Ascent: distance from top to bottom of e.g. 'A' [0..LegacySize] (unscaled)
...
// - If you used g.Font + g.LegacySize (which is frequent from the ImGui layer), you can use g.FontBaked as a shortcut, as g.FontBaked == g.Font->GetFontBaked(g.LegacySize).
struct ImFont
{
...
namespace ImGui
{
    ...
    IMGUI_API void      SetWindowFontScale(float scale);                        // Set font scale factor for current window. Prefer using PushFont(NULL, style.LegacySizeBase * factor) or use style.FontScaleMain to scale all windows.
    ...
    //static inline float GetWindowLegacySize()                   { return GetLegacySize(); }                                           // OBSOLETED in 1.48
