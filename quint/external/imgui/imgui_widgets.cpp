...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    const ImVec2 total_size = ImVec2(g.LegacySize + (label_size.x > 0.0f ? (label_size.x + style.FramePadding.x * 2) : 0.0f), label_size.y);  // Empty text doesn't add padding
    ...
    RenderBullet(window->DrawList, bb.Min + ImVec2(style.FramePadding.x + g.LegacySize * 0.5f, g.LegacySize * 0.5f), text_col);
    RenderText(bb.Min + ImVec2(g.LegacySize + style.FramePadding.x * 2, 0.0f), text_begin, text_end, false);
}
...
bool ImGui::ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags)
{
    ...
    RenderArrow(window->DrawList, bb.Min + ImVec2(ImMax(0.0f, (size.x - g.LegacySize) * 0.5f), ImMax(0.0f, (size.y - g.LegacySize) * 0.5f)), text_col, dir);
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    const ImRect bb(pos, pos + ImVec2(g.LegacySize, g.LegacySize));
    ...
    const float cross_extent = g.LegacySize * 0.5f * 0.7071f - 1.0f;
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    ImRect bb(pos, pos + ImVec2(g.LegacySize, g.LegacySize));
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = CalcItemSize(size_arg, CalcItemWidth(), g.LegacySize + style.FramePadding.y * 2.0f);
...
void ImGui::Bullet()
{
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const float line_height = ImMax(ImMin(window->DC.CurrLineSize.y, g.LegacySize + style.FramePadding.y * 2), g.LegacySize);
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(g.LegacySize, line_height));
    ItemSize(bb);
    if (!ItemAdd(bb, 0))
    {
    ...
    ImU32 text_col = GetColorU32(ImGuiCol_Text);
    RenderBullet(window->DrawList, bb.Min + ImVec2(style.FramePadding.x + g.LegacySize * 0.5f, line_height * 0.5f), text_col);
...
void ImGui::NewLine()
{
    ...
    if (window->DC.CurrLineSize.y > 0.0f)     // In the event that we are on a line with items that is smaller that LegacySize high, we will preserve its height.
        ItemSize(ImVec2(0, 0));
    else
        ItemSize(ImVec2(0.0f, g.LegacySize));
...
void ImGui::AlignTextToFramePadding()
{
    ...
    ImGuiContext& g = *GImGui;
    window->DC.CurrLineSize.y = ImMax(window->DC.CurrLineSize.y, g.LegacySize + g.Style.FramePadding.y * 2);
...
static float CalcMaxPopupHeightFromItemCount(int items_count)
{
    ...
    if (items_count <= 0)
        return FLT_MAX;
    return (g.LegacySize + g.Style.ItemSpacing.y) * items_count - g.Style.ItemSpacing.y + (g.Style.WindowPadding.y * 2);
}
...
static ImVec2 InputTextCalcTextSize(ImGuiContext* ctx, const char* text_begin, const char* text_end_display, const char* text_end, const char** out_remaining, ImVec2* out_offset, ImDrawTextFlags flags)
{
    ...
    return ImFontCalcTextSizeEx(g.Font, g.LegacySize, FLT_MAX, obj->WrapWidth, text_begin, text_end_display, text_end, out_remaining, out_offset, flags);
}
...
static int STB_TEXTEDIT_MOVELINESTART_IMPL(ImGuiInputTextState* obj, ImStb::STB_TexteditState* state, int cursor)
{
    ...
    if (obj->WrapWidth > 0.0f)
    {
        ...
        while (p >= p_bol)
        {
            const char* p_eol = ImFontCalcWordWrapPositionEx(g.Font, g.LegacySize, p, text_end, obj->WrapWidth, ImDrawTextFlags_WrapKeepBlanks);
...
static int InputTextLineIndexBuild(ImGuiInputTextFlags flags, ImGuiTextIndex* line_index, const char* buf, const char* buf_end, float wrap_width, int max_output_buffer_size, const char** out_buf_end)
{
    ...
    if (flags & ImGuiInputTextFlags_WordWrap)
    {
        for (s = buf; s < buf_end; s = (*s == '\n') ? s + 1 : s)
        {
            ...
            s = ImFontCalcWordWrapPositionEx(g.Font, g.LegacySize, s, buf_end, wrap_width, ImDrawTextFlags_WrapKeepBlanks);
        }
    }
...
static ImVec2 InputTextLineIndexGetPosOffset(ImGuiContext& g, ImGuiInputTextState* state, ImGuiTextIndex* line_index, const char* buf, const char* buf_end, int cursor_n)
{
    ...
    offset.y = (line_no + 1) * g.LegacySize;
    return offset;
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.LegacySize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    if (g.ActiveId == id)
    {
        ...
        const float mouse_y = (is_multiline ? (io.MousePos.y - draw_window->DC.CursorPos.y) : (g.LegacySize * 0.5f));
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const int row_count_per_page = ImMax((int)((inner_size.y - style.FramePadding.y) / g.LegacySize), 1);
        ...
        else if (IsKeyPressed(ImGuiKey_UpArrow) && is_multiline)     { if (io.KeyCtrl) SetScrollY(draw_window, ImMax(draw_window->Scroll.y - g.LegacySize, 0.0f)); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTSTART : STB_TEXTEDIT_K_UP) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_DownArrow) && is_multiline)   { if (io.KeyCtrl) SetScrollY(draw_window, ImMin(draw_window->Scroll.y + g.LegacySize, GetScrollMaxY())); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTEND : STB_TEXTEDIT_K_DOWN) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_PageUp) && is_multiline)      { state->OnKeyPressed(STB_TEXTEDIT_K_PGUP | k_mask); scroll_y -= row_count_per_page * g.LegacySize; }
        else if (IsKeyPressed(ImGuiKey_PageDown) && is_multiline)    { state->OnKeyPressed(STB_TEXTEDIT_K_PGDOWN | k_mask); scroll_y += row_count_per_page * g.LegacySize; }
    ...
    if (is_multiline)
        CalcClipRectVisibleItemsY(clip_rect, draw_pos, g.LegacySize, &line_visible_n0, &line_visible_n1);
    ...
    text_size = ImVec2(inner_size.x, line_count * g.LegacySize);
    //GetForegroundDrawList()->AddRect(draw_pos + ImVec2(0, line_visible_n0 * g.LegacySize), draw_pos + ImVec2(frame_size.x, line_visible_n1 * g.LegacySize), IM_COL32(255, 0, 0, 255));
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            if (is_multiline)
            {
                ...
                if (cursor_offset.y - g.LegacySize < scroll_y)
                    new_scroll_y = ImMax(0.0f, cursor_offset.y - g.LegacySize);
        ...
        {
            if (is_multiline)
                new_scroll_y = cursor_offset.y - g.LegacySize - (inner_size.y * 0.5f - style.FramePadding.y);
        ...
        if (render_selection)
        {
            ...
            for (int line_n = line_visible_n0; line_n < line_visible_n1; line_n++)
            {
                ...
                rect.Min.y = draw_pos.y - draw_scroll.y + line_n * g.LegacySize;
                rect.Max.x = rect.Min.x + rect_width;
                rect.Max.y = rect.Min.y + bg_offy_dn + g.LegacySize;
    ...
    if ((is_multiline || (buf_display_end - buf_display) < buf_display_max_length) && (text_col & IM_COL32_A_MASK) && (line_visible_n0 < line_visible_n1))
        g.Font->RenderText(draw_window->DrawList, g.LegacySize,
            draw_pos - draw_scroll + ImVec2(0.0f, line_visible_n0 * g.LegacySize),
    ...
    if (render_cursor)
    {
        ...
        ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y - g.LegacySize + 0.5f, cursor_screen_pos.x + 1.0f, cursor_screen_pos.y - 1.5f);
        ...
        if (!is_readonly && g.ActiveId == id)
        {
            ...
            ime_data->InputPos = ImVec2(cursor_screen_pos.x - 1.0f, cursor_screen_pos.y - g.LegacySize);
            ime_data->InputLineHeight = g.LegacySize;
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    ImVec2 sz(g.LegacySize * 3 + g.Style.FramePadding.y * 2, g.LegacySize * 3 + g.Style.FramePadding.y * 2);
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_picker)
    {
        ImVec2 picker_size(g.LegacySize * 8, ImMax(g.LegacySize * 8 - (GetFrameHeight() + g.Style.ItemInnerSpacing.x), 1.0f)); // FIXME: Picker size copied from main picker function
...
static void TreeNodeStoreStackData(ImGuiTreeNodeFlags flags, float x1)
{
    ...
    tree_node_data->DrawLinesX1 = draw_lines ? (x1 + g.LegacySize * 0.5f + g.Style.FramePadding.x) : +FLT_MAX;
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    const float text_offset_x = g.LegacySize + (display_frame ? padding.x * 3 : padding.x * 2);   // Collapsing arrow width + Spacing
    const float text_offset_y = ImMax(padding.y, window->DC.CurrLineTextBaseOffset);            // Latch before ItemSize changes it
    const float text_width = g.LegacySize + label_size.x + padding.x * 2;                         // Include collapsing arrow

    // We vertically grow up to current line height up the typical widget height.
    const float frame_height = ImMax(ImMin(window->DC.CurrLineSize.y, g.LegacySize + style.FramePadding.y * 2), label_size.y + padding.y * 2);
    ...
    const float arrow_hit_x2 = (text_pos.x - text_offset_x) + (g.LegacySize + padding.x * 2.0f) + style.TouchExtraPadding.x;
    ...
    // Render
    {
        ...
        if (display_frame)
        {
            ...
            if (flags & ImGuiTreeNodeFlags_Bullet)
                RenderBullet(window->DrawList, ImVec2(text_pos.x - text_offset_x * 0.60f, text_pos.y + g.LegacySize * 0.5f), text_col);
            else if (!is_leaf)
            ...
            if (flags & ImGuiTreeNodeFlags_ClipLabelForTrailingButton)
                frame_bb.Max.x -= g.LegacySize + style.FramePadding.x;
            if (g.LogEnabled)
                LogSetNextTextDecoration("###", "###");
        }
        else
        {
            ...
            if (span_all_columns && !span_all_columns_label)
                TablePopBackgroundChannel();
            if (flags & ImGuiTreeNodeFlags_Bullet)
                RenderBullet(window->DrawList, ImVec2(text_pos.x - text_offset_x * 0.5f, text_pos.y + g.LegacySize * 0.5f), text_col);
            else if (!is_leaf)
                RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + g.LegacySize * 0.15f), text_col, is_open ? ((flags & ImGuiTreeNodeFlags_UpsideDownArrow) ? ImGuiDir_Up : ImGuiDir_Down) : ImGuiDir_Right, 0.70f);
            if (g.LogEnabled)
                LogSetNextTextDecoration(">", NULL);
        }

        if (draw_tree_lines)
            TreeNodeDrawLineToChildNode(ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + g.LegacySize * 0.5f));
...
void ImGui::TreeNodeDrawLineToTreePop(const ImGuiTreeNodeStackData* data)
{
    ...
    if (data->TreeFlags & ImGuiTreeNodeFlags_DrawLinesFull)
    {
        ...
        y2_full = ImTrunc(y2_full - g.Style.ItemSpacing.y - g.LegacySize * 0.5f);
...
{
    ImGuiContext& g = *GImGui;
    return g.LegacySize + (g.Style.FramePadding.x * 2.0f);
}
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_visible != NULL)
    {
        ...
        float button_size = g.LegacySize;
...
static void BoxSelectScrollWithMouseDrag(ImGuiBoxSelectState* bs, ImGuiWindow* window, const ImRect& inner_r)
{
    ...
    for (int n = 0; n < 2; n++) // each axis
    {
        ...
        const float speed_multiplier = ImLinearRemapClamp(g.LegacySize, g.LegacySize * 5.0f, 1.0f, 4.0f, ImAbs(dist)); // x1 to x4 depending on distance
        const float scroll_step = g.LegacySize * 35.0f * speed_multiplier * ImSign(dist) * g.IO.DeltaTime;
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    else
    {
        ...
        float checkmark_w = IM_TRUNC(g.LegacySize * 1.20f);
        ...
        RenderArrow(window->DrawList, pos + ImVec2(offsets->OffsetMark + extra_w + g.LegacySize * 0.30f, 0.0f), GetColorU32(ImGuiCol_Text), ImGuiDir_Right);
    }
    if (!enabled)
        EndDisabled();
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        {
            const float ref_unit = g.LegacySize; // FIXME-DPI
...
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled)
{
    ...
    else
    {
        ...
        if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible)
        {
            ...
            if (selected)
                RenderCheckMark(window->DrawList, pos + ImVec2(offsets->OffsetMark + stretch_w + g.LegacySize * 0.40f, g.LegacySize * 0.134f * 0.5f), GetColorU32(ImGuiCol_Text), g.LegacySize * 0.866f);
        }
    }
...
bool    ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags)
{
    ...
    ImRect tab_bar_bb = ImRect(window->DC.CursorPos.x, window->DC.CursorPos.y, window->WorkRect.Max.x, window->DC.CursorPos.y + g.LegacySize + g.Style.FramePadding.y * 2);
    tab_bar->ID = id;
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    if (tab_bar->ScrollingAnim != tab_bar->ScrollingTarget)
    {
        ...
        tab_bar->ScrollingSpeed = ImMax(tab_bar->ScrollingSpeed, 70.0f * g.LegacySize);
        tab_bar->ScrollingSpeed = ImMax(tab_bar->ScrollingSpeed, ImFabs(tab_bar->ScrollingTarget - tab_bar->ScrollingAnim) / 0.3f);
        const bool teleport = (tab_bar->PrevFrameVisible + 1 < g.FrameCount) || (tab_bar->ScrollingTargetDistToVisibility > 10.0f * g.LegacySize);
...
static void ImGui::TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiID tab_id, ImGuiTabBarSection* sections)
{
    ...
    ImGuiContext& g = *GImGui;
    float margin = g.LegacySize * 1.0f; // When to scroll to make Tab N+1 visible always make a bit of N visible to suggest more scrolling area (since we don't have a scrollbar)
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    const ImVec2 arrow_button_size(g.LegacySize - 2.0f, g.LegacySize + g.Style.FramePadding.y * 2.0f);
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    const float tab_list_popup_button_width = g.LegacySize + g.Style.FramePadding.y;
    const ImVec2 backup_cursor_pos = window->DC.CursorPos;
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button_or_unsaved_marker)
{
    ...
    if (has_close_button_or_unsaved_marker)
        size.x += g.Style.FramePadding.x + (g.Style.ItemInnerSpacing.x + g.LegacySize); // We use Y intentionally to fit the close button circle.
    else
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    const float button_sz = g.LegacySize;
