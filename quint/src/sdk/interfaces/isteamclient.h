...
struct callbackname { \
	...
	const char *GetCallbackName() { return #callbackname; } \
	uint32  GetCallbackID() { return callbackname::k_iCallback; }

#define CALLBACK_MEMBER( varidx, vartype, varname ) \
	public: vartype varname ; \
	void GetMemberVar_##varidx( unsigned int &varOffset, unsigned int &varSize, uint32 &varCount, const char **pszName, const char **pszType ) { \
...
#define END_CALLBACK_INTERNAL_BEGIN( numvars )  \
	uint32  GetNumMemberVariables() { return numvars; } \
	bool    GetMemberVariable( uint32 index, uint32 &varOffset, uint32 &varSize,  uint32 &varCount, const char **pszName, const char **pszType ) { \
...
#define END_DEFINE_CALLBACK_0() \
	uint32  GetNumMemberVariables() { return 0; } \
	bool    GetMemberVariable( uint32 index, uint32 &varOffset, uint32 &varSize,  uint32 &varCount, const char **pszName, const char **pszType ) { REFERENCE( pszType ); REFERENCE( pszName ); REFERENCE( varCount ); REFERENCE( varSize ); REFERENCE( varOffset ); REFERENCE( index ); return false; } \
	};
