...
> Дата актуализации: 2026-09-03. Последняя сборка: **v80** (build CS2 14178),
> `release/neverwin_v80.dll` — собрана zig (x86_64-windows-gnu) на Linux.
> **v80 (2026-09-03):** диагностика гейта ESP (включил тумблер → сводная строка
> `esp: включён. localPlayer=... layout=...`; сломанный вход → warning 5 с;
> «ESP вкл, но меню не встало» логируется из feature loop — раньше три отказа
> на входе в DrawEsp уходили в лог МОЛЧА). В остальном = v79.
...
Если боксов НЕТ: 1) есть ли в логе строка `esp: включён. ...` — её НЕТ вовсе, если тумблер не включён (дефолт false) ИЛИ in-game меню не встало (свопчейн-хук — в логе `WARNING: свопчейн игры не найден`; этот случай логируется из feature loop: `esp: включён, но in-game меню не встало`). 2) В сводной строке `layout=FALLBACK` → смотреть `entity-list:` (dual-verify не подтвердил; скан не запускается). 3) `localPlayer=0` → нет павна (лобби) или стухший dwLocalPlayerPawn. 4) WARNING про view matrix. 5) обнови neverwin.ini.
Если боксы «не на игроках»: 1) `entity-list:` в логе (layout подтверждён?), 2) WARNING про view matrix, 3) обнови neverwin.ini.
Важно: `g_state.localPlayer` = `clientBase + dwLocalPlayerPawn` (статический глобал), НЕ зависит от entity-list layout — «aimbot жив, а ESP мёртв» — нормальная комбинация, они читают память разными путями.
